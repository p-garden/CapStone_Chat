...
def build_prompt_with_strategies(reaction_type: str, stage: str, approach: str) -> str:
    ...
    """
    reaction_prompt = load_prompt("반응", reaction_type)
    stage_prompt = load_prompt("단계", stage)
    approach_prompt = load_prompt("치료법", approach)

    # 디버깅 코드
    #print("🧩 [프롬프트 로드 결과]")
    #print(f"[단계 프롬프트] ({stage}):\n{stage_prompt[:50]}")
    #print(f"[반응 프롬프트] ({reaction_type}):\n{reaction_prompt[:50]}")
    #print(f"[접근 프롬프트] ({approach}):\n{approach_prompt[:50]}")
